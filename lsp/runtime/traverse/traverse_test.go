...
func (suite *TraverseSuite) TestPyFirstFunction() {
	...
	functionName := ""
	machine := machine.NewMachine(pythonTest)
	state := machine.Start()
	traverse.Traverse(targetTree, func(node *tree_sitter.Node) bool {
		state = state(node)
		if machine.HasSymbol && machine.Symbol.Is(symbol.FunctionKind) {
...
func (suite *TraverseSuite) TestPrintClassKinds() {
	...
	traverse.Traverse(classTree, func(node *tree_sitter.Node) bool {
		fmt.Println(node.Kind())
